...
export default function Form() {
  ...
  const [search, update_search] = React.useState(false)
  ...
  const Submit_data = () => {
    ...
    if(get_item !==null)
    {
      ...
      localStorage.setItem('Form_Items',JSON.stringify([...store,objectdata]))

    }
    update_arr([...Arr_data, objectdata])
    // update_localstore([...localstore,objectdata])
  ...
  const edit_data = (field_items,ID) => {
    ...
    updateobject({ Name: field_items.Name, Address: field_items.Address, City: field_items.City })
  ...
  const search_data = (e) => {
    ...
    const return_arr = parse_data.filter(Element => {
      return Element.Name.toLowerCase().indexOf(e.target.value.toLowerCase()) !== -1 || Element.Address.toLowerCase().indexOf(e.target.value.toLowerCase()) !== -1 || Element.City.toLowerCase().indexOf(e.target.value.toLowerCase()) !== -1
    }
    )
  ...
  return <>
    ...
    <div className="main_form">
      ...
      {
        search ? <input type="search" placeholder="search person" onChange={search_data}></input> : null
      }
        ...
        <tbody>
          {
            ...
            (
              <tr key={index}>
                ...
                <td>{field.City}</td>
                <td>
                  <button onClick={() => edit_data(field,field.uni_id)}><i class="fa-regular fa-pen-to-square"></i></button>
                  <button onClick={() => deletedata(field, index, field.uni_id)}><i class="fa-solid fa-trash-can"></i></button>
                </td>
